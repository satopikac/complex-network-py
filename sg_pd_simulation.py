...
def run_simulation_fast(neighbors: np.ndarray, degrees: np.ndarray, b: float, 
                        generations: int, transient: int, game_type: str) -> float:
    ...
    return np.float64(avg_cooperation) / np.float64(count) if count > 0 else np.float64(0)
...
def main():
    ...
    parser.add_argument('--game', '-g', default='SG', choices=['PD', 'SG'], 
    ...
    title_suffix = " (PD)" if game_type == "PD" else " (SG)"
    ...
    run_simulation_fast(dummy_neighbors, dummy_degrees, 0.5, 10, 5, game_type)
    ...
    for nt in network_types:
        ...
        for b in b_values:
            ...
            for _ in range(repeats):
                ...
                avg_coop += run_simulation_fast(neighbors, degrees, b, generations, transient, game_type)
    ...
    # 保存结果到CSV
    csv_filename = f"sg_results_{game_type.lower()}.csv"
    ...
    plt.title(f'simulation result(z=4){title_suffix}')
    plt.legend()
    plt.grid(True)
    plt.savefig(f'cooperation_results_{game_type.lower()}.png')
